...
int main(int argc, char** argv) {
    ...
    // Wait for a connection.
        ...
        select(n_fd, &working_set, NULL, NULL, NULL);
        for(int i = 0; i < n_fd; i++){
            ...
            if(i == server.listen_fd){
                ...
                */
                ...
                ret = read_header_and_file(&requestP[conn_fd], &err);
                //printf("%s %s", requestP[conn_fd].query, requestP[conn_fd].file);
                if(ret > 0)  continue;
                else if(ret < 0) {
                    ...
                    nwritten = write(requestP[conn_fd].conn_fd, requestP[conn_fd].buf,
